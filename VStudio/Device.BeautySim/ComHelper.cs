...
namespace Device.BeautySim
{
    ...
    public class Win32DeviceMgmt
    {
        ...
        public static List<DeviceInfo> GetAllCOMPorts()
        {
            ...
            {
                throw new Exception(Language.str_fail_device_info);
            }
            ...
            try
            {
                ...
                while (true)
                {
                    ...
                    try
                    {
                        deviceInfo.hardwareID = GetDeviceHardwareID(hDeviceInfoSet, deviceInfoData);
                    }
                    catch (Exception ex1)
                    {
        ...
        private static string GetDeviceName(IntPtr pDevInfoSet, SP_DEVINFO_DATA deviceInfoData)
        {
            ...
            {
                throw new Exception(Language.str_fail_reg_key);
            }
            ...
            try
            {
                ...
                if (result != 0)
                {
                    throw new Exception(Language.str_cant_read_port_name + " " + deviceInfoData.ClassGuid);
                }
            }
            finally
            {
        ...
        private static string GetDeviceHardwareID(IntPtr hDeviceInfoSet, SP_DEVINFO_DATA deviceInfoData)
        {
            StringBuilder descriptionBuf = new StringBuilder(256);
            uint propRegDataType;
            uint length = (uint)descriptionBuf.Capacity;
            bool success = SetupDiGetDeviceRegistryProperty(hDeviceInfoSet, ref deviceInfoData, SPDRP_HARDWAREID,
                out propRegDataType, descriptionBuf, length, out length);
            if (!success)
            {
                throw new Exception(Language.str_cant_read_port_name + " " + deviceInfoData.ClassGuid);
